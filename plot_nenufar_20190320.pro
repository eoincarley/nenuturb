...
pro plot_nenufar_typeII, save=save, postscript=postscript
	...
	for i=0, 2 do begin	
	  ...
	  trebin = (size(data))[1]/3
	  frebin = (size(data))[2]/4
	  data = reverse(data, 2)
	  freq = reverse(freq)
	  ;-------------------------------------------;
	  ;	      Rebin if necessary
	  ...
	  if keyword_set(postscript) then begin
		setup_ps, './nfar_'+time2file(utimes[0])+'_sun_tracking.eps'
	  endif else begin
	  ...
	  loadct, 74
          reverse_ct
	  spectro_plot, alog10(data)>5.5<8.0, utimes, freq, /xs, /ys, $
