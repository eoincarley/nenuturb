...
pro read_nfar_data, file, t0, t1, f0, f1, data=data, utimes=utimes, freq=freq
   	READ_NU_SPEC, file, data,time,freq,beam,ndata,nt,dt,nf,df,ns, $
                tmin=t0*60.0, tmax=t1*60.0, fmin=f0, fmax=f1, fflat=3, ntimes=4
...
pro psd_typeIIb, save=save, plot_ipsd=plot_ipsd, postscript=postscript, rebin=rebin
...
;	data = apply_response(data, freq)
	...
	if ~keyword_set(postscript) then $
        ...
        compute_all_psds, data, utimes, freq, $
                ...
                pspecerr=pspecerr, sigcuts=sigcuts, component=1, psdsmooth=0.002, pval=1.0
	...
	loadct, 0
	if ~keyword_set(postscript) then window, 1, xs=700, ys=700	
	;-----------------------------------;
    	;
    	;       Plot mean PSD
    	;
